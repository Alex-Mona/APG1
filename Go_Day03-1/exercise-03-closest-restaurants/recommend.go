...
func handlerLogin(c *gin.Context, indexHtml string, places []Source, totalRecord int) {
	tmpl := template.Must(template.New("indexHtml").Parse(indexHtml))
	pageSize := 10
	// pageNum := c.Param("pageNum")
	pageNum := c.DefaultQuery("page", "")
...
func main() {
	...
	indexHtmlBytes2, err2 := ioutil.ReadFile("authorization.html")
	if err2 != nil {
		fmt.Println("Ошибка при чтении файла:", err)
		return
	}

	indexHtml2 := string(indexHtmlBytes2)
	...
	router.GET("/login", func(c *gin.Context) {
		handlerLogin(c, indexHtml2, places, totalRecord)
	})
