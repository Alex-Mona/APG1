...
func main() {
	...
	candyType := flag.String("k", "", "two-letter abbreviation for the candy type") // Тип конфет
	candyCount := flag.Int("c", 0, "count of candy to buy")                         // Количество конфет
	money := flag.Int("m", 0, "amount of money given to machine")                   // Количество денег
	flag.Parse()

	// Проверка, были ли переданы все необходимые флаги
	if !isFlagPassed("k") || !isFlagPassed("c") || !isFlagPassed("m") {
		log.Fatalln("Missing required arguments") // Завершаем программу, если не все флаги заданы
	}
	...
	if err != nil {
		log.Fatalf("Error reading root CA certificate: %v", err) // Завершаем программу, если ошибка чтения сертификата
	}
	rootCAPool := x509.NewCertPool()
	rootCAPool.AppendCertsFromPEM(rootCA) // Добавляем сертификаты в пул

	// Создание клиента HTTP с настройками TLS
	client := http.Client{
		...
		Transport: &http.Transport{
			...
			TLSClientConfig: &tls.Config{
				...
				GetClientCertificate: func(info *tls.CertificateRequestInfo) (*tls.Certificate, error) {
					...
					if err != nil {
						log.Printf("Error loading key pair: %v", err) // Логируем ошибку загрузки ключей
						return nil, err
					}
					return &c, nil
				},
